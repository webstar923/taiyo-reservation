...
const DashboardPage = () => {
  const { getWorkData, changeWork, createWork, deleteWork, getFlatData } = useDashboard();
                ...
                <div className="space-y-4">
                  ...
                  >
                    {works.map((work, index) => (
                      <MenuItem key={`${work.work_name}-${index}`} value={work.work_name}>
                        {work.work_name}
                      </MenuItem>
                    ))}
                  </TextField>
                  ...
                  <div className='w-full flex gap-3'>
                    ...
                    <TextField
                      ...
                      onChange={(e) => setKeyManagement(e.target.value as "事前用意" | "弊社管理")}
                  ...
                  <div className="w-full flex gap-3">
                    <FormGroup className="w-1/2">
                      {checkboxOptions.slice(0, Math.ceil(checkboxOptions.length / 2)).map(({ key, label }) => (
                        <FormControlLabel
                          key={key}
                          control={
                            <Android12Switch
                              checked={checkboxList[key as keyof typeof checkboxList]}
                              onChange={(e) =>
                    ...
                    <FormGroup className="w-1/2">
                      {checkboxOptions.slice(Math.ceil(checkboxOptions.length / 2)).map(({ key, label }) => (
                        <FormControlLabel
                          key={key}
                          control={
                            <Android12Switch
